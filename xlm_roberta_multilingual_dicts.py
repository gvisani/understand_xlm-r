...
if __name__ == '__main__':
    ...
    parser.add_argument('--individual', default=True)
    args = parser.parse_args()

    if type(args.individual) == bool:
        individual = args.individual
    elif args.individual.lower() == 'true':
        individual = True
    else:
        individual = False
    ...
    else: ## Each word constitutes its own sentence
        for language in dictionary:
            embeddings[language] = []
            for word_i, word in enumerate(dictionary[language]):
                ...
                toks = tok_map[0]
                start_cnt = int(np.sum([len(l) for l in tok_map[:0]]))+1
                #print("at word {}".format(word_i))
                for emb_i in range(len(outputs.hidden_states)):
                    embeddings[language][word_i]['Layer %d' % (emb_i)] = 0.0
                    for cnt in range(len(toks)):
